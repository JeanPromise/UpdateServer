...
def upload_apk():
    ...
    # Broadcast update
    socketio.emit("apk_update", apk_info, broadcast=True)
...
def handle_connect():
    ...
    print(f"[{datetime.utcnow().strftime('%H:%M:%S')}] Client connected")
...
def handle_register(data):
    ...
    if not any(u["device_id"] == device_id for u in users):
        ...
        print(f"[{datetime.utcnow().strftime('%H:%M:%S')}] Device registered: {device_id}")

@socketio.on("disconnect")
def handle_disconnect():
    print(f"[{datetime.utcnow().strftime('%H:%M:%S')}] Client disconnected")
